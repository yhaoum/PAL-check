...
RUN_LEVEL = 1
s=1*2/3 #shrunk for rw.sd
NP_MIF       = switch(RUN_LEVEL, 2, 10000)
...
sir.step <- Csnippet("
                     ...
                     while(q1<0 || q1>1){
                      q1 = rnorm(mu_q1, sigma_q);
                     }
                     while(q2<0 || q2>1){
                      q2 = rnorm(mu_q2, sigma_q);
                     }
                     while(q3<0 || q3>1){
                      q3 = rnorm(mu_q3, sigma_q);
                     }
                     ")
...
params["y2"] <- 628
params["y3"] <- 174
params["mu_q1"] <- 0.0429502
params["mu_q2"] <- 0.07887504
params["mu_q3"] <- 0.06917179
params["sigma_q"] <- 0.021

sir_fixed_params <- c(params_fixed, params["y1"], params["y2"], params["y3"])

pt <- pomp::parameter_trans(
  log = c("beta1","beta2","beta3","sigma","sigma_q"),
  logit=c("beta11","mu_q1","mu_q2","mu_q3"),
...
### Next-round code
el <- readRDS("output/binom/unknown_reporting/round_01/mif2_panel_Yize_el_report_binom_01")
x <- na.omit(el$fits)
...
mifs_global <- foreach(i=1:cores,.packages='pomp', .options.multicore=mcopts) %dopar% {
  panelPomp::mif2(
    ...
    rw.sd = rw_sd(beta1=0.002*s,beta2=0.002*s,beta3=0.002*s,
                  beta11=0.001*s,phi=0.01*s, sigma=0.01*s, 
                  mu_q1=0.001*s,mu_q2=0.001*s,mu_q3=0.001*s,
                  sigma_q=0.0002*s),
...
bake(file = "output/binom/unknown_reporting/round_02/mif2_panel_Yize_report_binom_dummy.rds",{ 
  mifs_global
})

bake(file = "output/binom/unknown_reporting/round_02/mif2_panel_Yize_el_report_binom_dummy.rds",{
