### This continues to run the following rounds of mif2
...
### Next-round code
el <- readRDS("output/binom/round_01/mif2_panel_Yize_el_report_binom_01")
x <- na.omit(el$fits)
...
mifs_global <- foreach(i=1:cores,.packages='pomp', .options.multicore=mcopts) %dopar% {
  panelPomp::mif2(
    ...
    rw.sd = rw_sd(beta1=0.002,beta2=0.002,beta3=0.002,
                  beta11=0.001,phi=0.01, sigma=0.01, sigma_q=0.0001),
...
bake(file = "output/binom/round_02/mif2_panel_Yize_report_nbinom_dummy.rds",{ 
  mifs_global
})

bake(file = "output/binom/round_02/mif2_panel_Yize_el_report_binom_dummy.rds",{
