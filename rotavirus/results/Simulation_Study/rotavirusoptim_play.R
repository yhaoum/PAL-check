...
load("sim_list_100+.Rdata")
prop <- 420

# realdat <- sim_list_100[[6]] |> as.matrix()
...
for(i in 1:100){
  ...
  lik_list <- rotavirus_SMC_qropxi(init_dist,realdat , 9, c(initial_guess[1],initial_guess[2],initial_guess[3],initial_guess[4],initial_guess[5]),norm_par = c(0.07,initial_guess[6]), gamma_par = c(initial_guess[7],  1/initial_guess[7]),t(prop), 10000,1)
  y[i] <- lik_list$log_lik
}

save(y, file = "wwr_100+_ovov.Rdata")
# maximization
