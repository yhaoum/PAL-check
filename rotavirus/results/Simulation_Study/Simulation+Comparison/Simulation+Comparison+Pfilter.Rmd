...
```{r}
sim_compare <- read.csv("sim_compare_WWR_EqOv.csv", header = T)
sim_compare_pomp <- read.csv("sim_compare_pomp_EqOv.csv", header = T)
```
...
for(i in 1:9){
  ...
  data <- melt(x)
  p.comparison[[i]] <- ggplot(data,aes(x=value, fill=.L1)) +     
    geom_density(alpha=0.25)+ labs(x = labx[i]) +
    ggtitle(paste("p = ", t.pvalue[i]))
}

pdf("compare_density_plot_EqOv.pdf",width=10,height=10)
