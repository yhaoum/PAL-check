...
pfilter(m1,Np=5000,params=theta,pred_mean(object = )  pred_mean=T, filter_mean=T,filter_traj=T,saved_states=T) -> pfs

for(k in 1:ncores){
  list_cond_lik[[k]] <- pfs[[k]]@cond.logLik
} 

x <- logmeanexp(logLik(pfs))  

save(list_cond_lik, file = "output/pfilter_condlik_ovov.Rdata")
save(x, file = "output/pfilter_ovov.Rdata")
