...
foreach(i=1:ncores, .combine=c,
  ...
  pfilter(m1,Np=np,save.states=T,params=theta)
} -> pfs  
...
for(i in 1:ncores){
  x <- c(0,0,0)
  for(j in 1:416){
    x <- rbind(x, rowMeans(pfs[[i]]@saved.states[[j]][c("H1","H2","H3"), ]))
  }
  filtered_list[[i]] <- x
}
...
save(filtered_list, file = "output/realdata/filtered_list_36.Rdata")
