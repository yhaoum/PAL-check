...
mat rotavirus_sim(int length,
  ...
  vec xi(4*length,fill::zeros);

  for(int t = 1; t<length; t++){
 // if(t==52){cout << state.row(t*4-5)<<endl;}
    for(int i = 0; i<4; i++){
      xi(4*(t)-4+i) = R::rgamma(sigma_xi, 1/sigma_xi);
      update_K(K,state.row(4*(t)-4+i).t(), betas.row(t-1).t(),xi(4*(t)-4+i),N, t, phase, amp);
      //cout << K << endl;
      for(int m = 0; m < 9; m++){
  ...
  for(int j = 0; j<3;j++){
    for(int t = 0; t<length-1; t++){
     double q = r_truncnorm(0.07, sigma_q,0,1);
     observations(t,j) = R::rbinom(weekly_incidence(t,j),q); 
    }
  }
